...
#'   set_prior(age ~ NFix(sd = 0.01)) |>
#'   set_prior(sex ~ NFix(sd = 0.01)) |>
#'   set_prior(time ~ NFix(sd = 0.01)) |>
...
#' vals_linpred <- bage:::make_linpred_from_components(mod = mod,
...
#' lambda <- replace(lambda, seq_along(lambda), lambda)
...
draw_vals_augment_unfitted.bage_mod <- function(mod) {
  ...
  ans$.observed <- ans[[nm_outcome_data]] / offset
