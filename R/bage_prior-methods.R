...
forecast_term.bage_prior_linar <- function(prior,
  ...
  if (zero_sum) {
    ...
    effect_forecast <- zero_sum_fitted(fitted = effect_forecast_old,
                                       along = along,
                                       dimnames_term = dimnames_forecast,
                                       var_time = var_time,
                                       var_age = var_age)
...
forecast_term.bage_prior_rwseasfix <- function(prior,
  ...
  if (zero_sum) {
    rw_forecast <- zero_sum_fitted(fitted = rw_forecast,
                                   along = along,
                                   dimnames_term = dimnames_forecast,
                                   var_time = var_time,
                                   var_age = var_age)
    seas_forecast <- zero_sum_fitted(fitted = seas_forecast,
                                     along = along,
                                     dimnames_term = dimnames_forecast,
                                     var_time = var_time,
                                     var_age = var_age)
...
forecast_term.bage_prior_rw2seasfix <- function(prior,
  ...
  seas_forecast <- forecast_seasfix(n_seas = n_seas,
                                    ...
                                    matrix_along_by_forecast = matrix_along_by_forecast)
  if (zero_sum) {
    rw2_forecast <- zero_sum_fitted(fitted = rw2_forecast,
                                    along = along,
                                    dimnames_term = dimnames_forecast,
                                    var_time = var_time,
                                    var_age = var_age)
