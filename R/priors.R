...
#' | [RW()]    | Random walk. Elements have a natural ordering, and neighbouring elements more strongly correlated than distant elements. | Main effects for age, time, cohort, or other smoothly-varying dimensions. | Yes |
...
#' | [ESVD()]  | `SVD()` prior within each combination of the 'by' variables. | Interaction between age and non-sex, non-gender variabes. | No |
#' | [ESVDS()] | `SVDS()` prior within each combination of the 'by' variables. | Interaction between age, sex/gender, and one or more other variables. | No |
