...
\begin{align}
  \beta_{u,1}^{(m)} & \sim \text{N}\left(0, (A_0^{(m)})^2\right) \\
  \beta_{u,v}^{(m)} & \sim \text{N}(\phi_m \beta_{u,v-1}^{(m)}, \tau_m^2), \quad v = 2, \cdots, V_m \\
  \phi_m & = a_{0m} + (a_{1m} - a_{0m}) \phi_m^{\prime} \\
  \phi_m^{\prime} & \sim \text{Beta}(S_1^{(m)}, S_2^{(m)})
\end{align}

$A_0^{(m)}$ can be 0, implying that $\beta_{u,1}^{(m)}$ is fixed at 0.

We require that $-1 \le a_{0m} < a_{1m} \le 1$.
...
\begin{equation}
  \text{N}(\tau_m \mid 0, A_{\tau}^{(m)2}) \text{Beta}( \phi_m^{\prime} \mid S_1^{(m)}, S_2^{(m)}) \prod_{u=1}^{U_m} \text{N}\left(\beta_{u,1}^{(m)} \mid 0, (A_0^{(m)})^2\right) \prod_{v=2}^{V_m} \text{N}\left(\beta_{u,v}^{(m)} \mid \phi_m \beta_{u,v-1}^{(m)}, \tau_m^2 \right) 
\end{equation}

### Forecasting

\begin{equation}
  \beta_{u,V_m+h}^{(m)} \sim \text{N}(\phi_m \beta_{u,V_m+h-1}^{(m)}, \tau_m^2)
\end{equation}
...
\begin{align}
  \beta_{u,1}^{(m)} & \sim \text{N}\left(0, (A_0^{(m)})^2\right) \\
  \beta_{u,2}^{(m)} & \sim \text{N}\left(\beta_{u,1}, (A_{\eta}^{(m)})^2\right) \\
  \beta_{u,v}^{(m)} & \sim \text{N}\left(\beta_{u,v-1}^{(m)} + \phi_m (\beta_{u,v-1}^{(m)} - \beta_{u,v-2}^{(m)}), \tau_m^2\right), \quad v = 3, \cdots, V_m \\
  \phi_m & = a_{0m} + (a_{1m} - a_{0m}) \phi_m^{\prime} \\
  \phi_m^{\prime} & \sim \text{Beta}(S_1^{(m)}, S_2^{(m)}) \\
  \tau_m & \sim \text{N}^+\left(0, (A_{\tau}^{(m)})^2\right)
\end{align}
...
\begin{align}
  & \text{N}(\tau_m \mid 0, A_{\tau}^{(m)2}) \text{Beta}( \phi_m^{\prime} \mid S_1^{(m)}, S_2^{(m)}) \prod_{u=1}^{U_m} \text{N}(\beta_{u,1}^{(m)} \mid 0, (A_0^{(m)})^2) \text{N}(\beta_{u,2}^{(m)} \mid \beta_{u,1}^{(m)}, (A_{\eta}^{(m)})^2) \\
  & \quad \quad \times \prod_{v=3}^{V_m} \text{N}\left(\beta_{u,v}^{(m)} \mid  \beta_{u,v-1}^{(m)} + \phi_m (\beta_{u,v-1}^{(m)} - \beta_{u,v-2}^{(m)}), \tau_m^2 \right) 
\end{align}
...
\begin{equation}
  \beta_{u,V_m+h}^{(m)} \sim \text{N}( \beta_{u,V_m+h-1}^{(m)} + \phi_m (\beta_{u,V_m+h-1}^{(m)} - \beta_{u,V_m+h-2}^{(m)}), \tau_m^2)
\end{equation}
...
# helper to build an aligned display-math block
align_block <- function(...) {
  lines <- c(...)
  body <- paste(lines, collapse = " \\\\ ")
  paste0("$$\\begin{aligned}", body, "\\end{aligned}$$")
}
...
models <- tibble(
  ...
  `System model` = c(
    ...
    # Poisson noise (single line; aligned not strictly needed)
    "$$ y_i^{\\text{true}} \\sim \\text{Poisson}(\\mu_i w_i) $$",

    # Poisson exposure (single line)
  ...
  `Data model` = c(
    ...
    # Poisson exposure
    align_block(
      "w_i^{\\text{obs}} \\sim \\text{InvGamma}(2 + d_{g[i]}^{-1},",
      "  [1 + d_{g[i]}^{-1}] w_i^{\\text{true}})"
    ),
...
## Poisson with undercount in outcome variable {#sec:datamod-pois-under}
...
## Poisson with overcount in outcome variable {#sec:datamod-pois-over}
...
## Poisson with undercount and overcount in outcome variable {#sec:datamod-pois-miscount}
...
## Poisson with noise added to outcome variable {#sec:datamod-pois-noise}
...
## Poisson with measurement errors in exposure {#sec:datamod-pois-off}
...
## Binomial with measurement errors in outcome variable {#sec:datamod-binom}
...
## Normal with noise added to outcome variable
...
## Filtering {#sec:filter-ag}

The data that we supply to TMB is a a filtered version of the data that the user provides through the `data` argument. We remove any rows where (i) the offset is 0 or NA, or (ii) the outcome variable is NA.


## Inner-Outer Approximation

The inner-outer approximation cannot be used with models that include data models.

### Step 0: Select 'inner' and 'outer' variables

Select variables to be used in inner model. By default, these are the age, sex, and time variables in the model. All remaining variables are 'outer' variables. Any covariates specified through `set_covariates()` are also treated as 'outer' variables.

### Step 1: Fit inner model

In Poisson and binomial models we set dispersion $\xi$ to 0.

We aggregate the data using the classification formed by the inner variables. We identify any rows in the data where there are duplicated combinations of the inner variables. For instance, if the inner variables are `age` and `sex`, and we have two rows where `age` is `"20-24"` and sex is `"Female"`, then these rows would count as duplicated combinations. We aggregate offset and outcome variables across these duplicates. Let $D$ be the number of times a particular combination is duplicated. 
...
We remove all terms not involving 'inner' variables, other than the intercept term, from the model. We fit the resulting model.
...
Aggregate the data using the classification formed by the outer variables and any covariates. Remove all terms involving the 'inner' variables, plus the intercept, from the model. Set exposure to $w_i^{\text{out}} = \hat{\mu}^{\text{in}} w_i$. Set dispersion to 0. Fit the model.

**Binomial model**

Do not aggregate. Fit the original model, but with dispersion set to zero, and using Known priors for 'inner' terms, based on point estimates from the inner model.

**Normal model**

Aggregate the data using the classification formed by the outer variables and any covariates. Remove all terms involving the 'inner' variables, plus the intercept, from the model. Set the outcome variable to to $y_i^{\text{out}} = y_i - \hat{\mu}^{\text{in}}. Fit the model.
...
### Derivation of $y_i^{\text{true}} \mid y_i^{\text{obs}}, \pi_{g[i]}, \mu_i, \xi, w_i$

Derivation of formula from Section \@ref(sec:datamod-pois-under)
...
### Derivation of $y_i^{\text{true}} \mid y_i^{\text{obs}}, \kappa_{h[i]}, \mu_i, \xi, w_i$ in overcount-only model

Derivation of formula from Section \@ref(sec:datamod-pois-over).
...
### Derivation of $u_i \mid y_i^{\text{obs}}, \pi_{g[i]}, \kappa_{h[i]}, \mu_i, \xi, w_i$

Derivation of formula from Section \@ref(sec:datamod-pois-miscount).
...
### Derivation of $y_i^{\text{true}} \mid u_i, \pi_{g[i]}, \kappa_{h[i]}, \mu_i, w_i, \xi$
...
### Derivation of $p(y_i^{\text{true}} \mid y_i^{\text{obs}}, \lambda, \mu_i, w_i)$

Derivation of formula from Section \@ref(sec:datamod-pois-noise).
...
Derivation of formula from Section \@ref(sec:datamod-binom).
