...
pkgdown:
  as_is: true
vignette: >
  %\VignetteIndexEntry{Modelling Mortality}
...
```
Population counts for the very old are sometimes unreliable. We modify the problematic zeros under the assumption that each person lives 0.5 person years before dying.
```{r}
dth <- dth |>
...
\begin{equation}
  \beta_j^{(2)} \sim \text{N}(0, 1) \quad j = 1, 2 \\
\end{equation}
...
Finally, the dispersion term $\xi$ has an prior
\begin{equation}
    p(\xi) = e^{-\xi}.
\end{equation}
