...
**bage** (= "Bayesian" + "age") implements Bayesian hierarchical models for rates, probabilities, and means, where the rates, probabilities and means are classified by variables such as age, sex, and region. Models in **bage** can be used for estimating past values and forecasting future values. 

The models in **bage** are hierarchical because they are built out of smaller sub-models. A model for mortality rates, for instance, might contain sub-models describing how rates vary over age, sex, and time. The models are Bayesian because, like Bayesian models more generally, they use probability distributions to represent uncertainties. 

Under the hood, **bage** draws on package [TMB](https://CRAN.R-project.org/package=TMB) to fit the models. **TMB** is fast and can handle large datasets -- characteristics that are inherited by **bage**.
...
We begin by loading the packages that we will need for the analysis of the injuries data. Loading **bage** automatically loads package [rvec](https://CRAN.R-project.org/package=rvec), which contains functions for  working with draws from probability distributions. Package [poputils](https://CRAN.R-project.org/package=poputils) contains functions for working with demographic data. Packages **dplyr** and **tidyr** are core [tidyverse](tidyverse.org) packages for manipulating data. We use **ggplot2** to graph inputs and outputs.
...
In this vignette, we will analyse a dataset called `nzl_injuries` that is included in the **bage** package. The dataset contains counts of injuries and population in New Zealand, classified by age, sex, ethnicity, and year.
...
The first argument to `mod_pois()` is an R-style formula identifying the outcome variable and the combinations of predictor variables to be used in the model. The second argument is the name of the dataset. The third argument is the name of the exposure variable. 
...
The table in the the middle of the printout above shows the default prior distribution assigned to each term. We discuss these distributions in Section \@ref(sec:priors).

The bottom row of the printed object shows various model settings. `n_draw` is the number of random draws produced by extractor functions such as `augment()`. We discuss these random draws further in Section \@ref(sec:outputs). `var_time`, `var_age`, and `var_sexgender` are the names of variables in dataset `nzl_injuries`. **bage** treats variables representing time, age and sex/gender specially, and attempts to identify these variables in input data, using variables names. If **bage** guesses wrong, the correct names can be supplied using functions such as `set_var_time()`. 
...
Note that replacing an existing prior reverts a model to 'unfitted' status. 
