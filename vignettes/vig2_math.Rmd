...
```
RW(s = 1, sd_intercept = 0.001)
```

- `s` is $A_{\tau}^{(m)}$. Defaults to 1.
- `sd_intercept` is A_{\eta}^{(m)}$. Defaults to 0.001.
...
```
RW2(s = 1, sd_intercept = 0.001, sd_slope = 1)
```

- `s` is $A_{\tau}^{(m)}$. Defaults to 1.
- `sd_intercept` is $A_{\eta,0}^{(m)}$. Defaults to 0.001.
- `s_slope` is $A_{\eta,1}^{(m)}$. Defaults to 1.
...
\begin{align}
  ...
  \epsilon_j^{(m)} & \sim \text{N}\left(\phi_m \epsilon_{j-1}^{(m)}, (1 - \phi_m^2) \tau_m^2\right), \quad j = 2, \cdots, J_m \\
  \phi_m & = a_{0,m} + (a_{1,m} - a_{0,m}) \phi_m^{\prime} \\
  \phi_m^{\prime} & \sim \text{Beta}(2, 2) \\
  \tau_m & \sim \text{N}^+\left(0, A_{\tau}^{(m)2}\right)
\end{align}
...
\begin{equation}
  \text{N}(\tau_m | 0, A_{\tau}^{(m)2}) \text{Beta}( \phi_m^{\prime} | 2, 2) \text{N}\left(\epsilon_1^{(m)} \mid 0, \tau_m^2 \right)  \text{N}(\eta_m | 0, A_{\eta}^{(m)2}) \prod_{j=2}^{J_m} \text{N}\left(\epsilon_j^{(m)} \mid \phi_m \epsilon_{j-1}^{(m)}, (1 - \phi_m^2) \tau_m^2  \right) 
\end{equation}
...
\begin{equation}
  \text{N}(\tau_m | 0, A_{\tau}^{(m)2}) \text{N}\left( {\textstyle\sum}_{k=1}^{K_m} \alpha_k^{(m)} \mid 0, 1 \right)  \prod_{k=3}^{K_m} \text{N}\left(\alpha_k^{(m)} - 2 \alpha_{k-1}^{(m)} + \alpha_{k-2}^{(m)} \mid 0, \tau_m^2 \right) 
\end{equation}
...
\begin{align}
  \lambda_{j}^{(m)} & \sim \text{N}(\lambda_{j - S_m}^{(m)}, \kappa_m^2 ) \\ 
  \kappa_m & \sim \text{N}^+\left(0, A_{\kappa}^{(m)2}\right) \\
...
\begin{equation}
  \text{N}(\kappa_m | 0, A_{\kappa}^{(m)2}) \text{N}\left( {\textstyle\sum}_{j=1}^{J_m} \lambda_j^{(m)} \mid 0, 1 \right)  \prod_{j=2}^{J_m} \text{N}\left(\lambda_j^{(m)} - \lambda_{j-1}^{(m)} \mid 0, \kappa_m^2 \right) 
\end{equation}
