...
\begin{equation}
  \beta^{(0)} \sim \mathcal{N}(0, A_0^2)
\end{equation}

By default, $A_0 = 10$, which gives the intercept a proper but diffuse prior.
...
- `scale` is $A_{\tau}^{(m)}$. Defaults to 1.
...
\begin{align}
  \beta_j^{(m)} & = h_j^{(m)} \eta_m + \epsilon_j^{(m)} \\
   h_q^{(m)} & = -\frac{J_m + 1}{J_m - 1} + \frac{2}{J_m - 1} j \\
...
- `scaled_svd` contains $\pmb{F}^{(m)}$ and $\pmb{g}^{(m)}$
- `n` is the number of components
- `indep` Type of age-sex interaction TODO - COMPLETE
...
Seasonal effects are modelled by a vector $\pmb{\lambda}^{(m)}$ with $J_m$ elements. Seasonal effects are divided into $S_m$ batches, corresponding to $S_m$ "seasons", where $2 \le S_m \le J_m/2$. Each batch of seasonal effects follows a random walk, and there is a soft constrain on the sum across all batches.
