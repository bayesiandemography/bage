...
models <- tibble(
  Model = c("Poisson", "Poisson", "Poisson", "Binomial", "Normal"),
  `System model` = c(
    "$$ y_i^{\\text{true}} \\sim \\text{Poisson}(\\mu_i w_i) $$",
    "$$ y_i^{\\text{true}} \\sim \\text{Poisson}(\\mu_i w_i) $$",
    "$$ y_i \\sim \\text{Poisson}(\\mu_i w_i^{\\text{true}}) $$",
    "$$ y_i^{\\text{true}} \\sim \\text{Binomial}(w_i, \\mu_i) $$",
  ...
  `Data model` = c(
    ...
    paste(
      ...
      "\\epsilon_i \\sim \\text{Skellam}(\\lambda_{g[i]}, \\lambda_{g[i]})",
      "$$"
    ),
    paste(
      "$$",
...
No dispersion term (ie $\xi \equiv 0$), and be explicit that modelling actual outcome:
\begin{equation}
  y_i^{\text{true}} \sim \text{Poisson}(\mu_i w_i). (\#eq:lik-pois-outcome)
\end{equation}
...
\begin{align}
  y_i^{\text{obs}} & = u_i + v_i \\
  u_i & \sim \text{Binomial}(y_i^{\text{true}}, \pi_i) (\#eq:datamod-u-i) \\
  v_i & \sim \text{Poisson}(\kappa_i \gamma_i w_i) (\#eq:datamod-v-i) \\
  \pi_i & \sim \text{Beta}(A_g^{(\pi)}, B_g^{(\pi)}) \\ 
  \kappa_i & \sim \text{Gamma}(A_h^{(\kappa)}, B_h^{(\kappa)}),
\end{align}
where values for $A_g^{(\pi)}$, $B_g^{(\pi)}$, $A_h^{(\kappa)}$, and $B_h^{(\kappa)}$ are supplied by the user.

Note
\begin{align}
  u_i & \mid \pi_i, \gamma_i, w_i \sim \text{Poisson}(\pi_i \gamma_i w_i), (\#eq:datamod-u-i-cond) \\
  y_i^{\text{obs}} & \mid \pi_i, \kappa_i, \gamma_i, w_i \sim \text{Poisson}([\pi_i + \kappa_i] \gamma_i w_i)
\end{align}
...
\begin{align}
  y_i^{\text{obs}} & \sim \begin{cases}
    \text{NegBinom}\left(\xi^{-1}, [1 + (\pi_i + \kappa_i)\mu_i w_i \xi]^{-1}\right) & \xi > 0 \\
    \text{Poisson}([\pi_i + \kappa_i]\mu_i w_i) & \xi \equiv 0
  \end{cases} \\
  \pi_i & \sim \text{Beta}(A_g^{(\pi)}, B_g^{(\pi)}) \\
  \kappa_i & \sim \text{Gamma}(A_h^{(\kappa)}, B_h^{(\kappa)})
\end{align}
