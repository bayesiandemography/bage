...
Matrix $\pmb{Z}$ is a standardized version of the original covariate data supplied by the user. If a covariate is a numeric variable, it is standardized to have mean 0 and standard deviation one. If it is a categorical variable with $C$ categories, it is converted into $C-1$ indicator variables, with the first category as the omitted variable. (This is the 'treatment' contrast in R.)
...
- `n_nonzero` Effective number of non-zero coefficients. If a value is supplied, the shrinkage prior is used; otherwise the standard prior is used.
