...
## Random walk with drift, with seasonal effect {#sec:rw2seas}
...
\begin{align}
  \beta_{u,1}^{(m)} & \sim \text{N}(0, 1), \quad u = 1,\cdots,U_m  \\
  \beta_{u,v}^{(m)} & \sim \text{N}(\beta_{u,1}^{(m)} + (v - 1) \eta_u^{(m)}, \tau_m^2), \quad u = 1,\cdots,U_m, \quad v = 2, \cdots, V_m  \\
...
\begin{equation}
  \text{N}(\tau_m \mid 0, A_{\tau}^{(m)2}) \prod_{u=1}^{U_m} \text{N}(\eta_u^{(m)} \mid 0, A_{\eta}^{(m)2}) \prod_{u=1}^{U_m} \text{N}(\beta_{u,1}^{(m)} \mid 0, 1) \prod_{u=1}^{U_m} \prod_{v=2}^{V_m} \text{N}\left(\beta_{u,v}^{(m)} | \beta_{u,1}^{(m)} + (v-1) \eta_u^{(m)}, \tau_m^2 \right) 
\end{equation}
...
\begin{equation}
  \beta_{u,V_m + h}^{(m)} \sim \text{N}(\beta_{u,1} + (V_m + h - 1) \eta_u^{(m)}, \tau_m^2)
\end{equation}
...
TODO - describe derivaition of components within LinAR, RWSeas, RW2Seas.
