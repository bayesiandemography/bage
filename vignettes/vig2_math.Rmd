...
\begin{align}
  y_i^{\text{obs}} & \sim \text{NegBinom}\left(\xi^{-1}, [1 + (\pi_{g[i]} + \kappa_{h[i]})\mu_i w_i \xi]^{-1}\right) \\
  \pi_g & \sim \text{Beta}(A_g^{(\pi)}, B_g^{(\pi)}) \\
  \kappa_h & \sim \text{Gamma}(A_h^{(\kappa)}, B_h^{(\kappa)})
\end{align}
...
\begin{equation}
  y_i^{\text{true}} - u_i | u_i, y_i^{\text{obs}}, \pi_{g[i]}, \mu_i, \xi, w_i \sim \text{NegBinom}\left(u_i + \xi^{-1}, \frac{1 + \pi_{h[i]} \mu_i w_i \xi}{1 + \mu_i w_i \xi} \right)
\end{equation}
...
Draw
\begin{equation}
 x_i = y_i^{\text{true}} - y_i^{\text{obs}} \sim \text{NegBinom}\left(y_i^{\text{obs}} + \xi^{-1s}, \frac{1 + \pi_{h[i]} \mu_i w_i \xi}{1 + \mu_i w_i \xi} \right)
\end{equation}
...
\begin{align}
  y_i^{\text{obs}} & \sim \text{BetaBinomial}(w_i, \xi^{-1} \pi_{g[i]} \mu_i, \xi^{-1} [1-\pi_{g[i]} \mu_i]) \\
  \pi_g & \sim \text{Beta}(A_g^{(\pi)}, B_g^{(\pi)})
\end{align}
...
Draw from distribution defined by
\begin{align}
...
\begin{align}
  & p(y_i^{\text{true}} \mid y_i^{\text{obs}}, \pi_{g[i]}, \mu_i, \xi, w_i) \\
