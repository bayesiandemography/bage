...
\begin{equation}
  \pmb{\beta}^{(m)} = \pmb{\beta}_{\text{trend}}^{(m)} + \pmb{\beta}_{\text{cyc}}^{(m)} + \pmb{\beta}_{\text{seas}}^{(m)}
\end{equation}
where

- $\pmb{\beta}_{\text{trend}}^{(m)}$ is a trend;
- $\pmb{\beta}_{\text{cyc}}^{(m)}$ is a cyclical effect;
- $\pmb{\beta}_{\text{seas}}^{(m)}$ is a seasonal effect.
...
We adopt notation from @gelman2014bayesian [p. 395]. Let $\beta_{j_i^m}^{(m)}$ denote the element of $\pmb{\beta}^{(m)}$ that contributes to $\mu_i$. Under our model of means,
\begin{equation}
...
Define $\phi_j^m$ as the set of $i$ such that $j_i^m = j$, and $n^{(m)}$ as the number of elements in $\phi_j^m$ (which is identical for $j = 1, \cdots, J_m$).

Algorithm for processing a single draw from the posterior distribution:

- Set $\tilde{\beta}^{(0)} = \sum_{i=1}^n \mu_i \big/ n$, set $r_i^{(0)} = \mu_i$, and set $r_i^{(1)} = r_i^{(0)} - \tilde{\beta}^{(0)}$
- For $m = 1, \cdots, M$: set $\tilde{\beta}_j^{(m)} = \sum_{i \in \phi_j^m} r_i^{(m)} \big/ n^{(m)}$ and set $r_i^{(m+1)} = r_i^{(m)} - \tilde{\beta}_{j_i^m}^{(m)}$.

Note that $r_i^{(M+1)} = 0$. **Or at least I think so. I can't figure out how to prove it.**

Let
\begin{equation}
  \tilde{\mu}_i = \sum_{m=0}^M \tilde{\beta}_{j_i^m}^{(m)}
\end{equation}

Then $\tilde{\mu}_i = \mu_i$. Proof.

\begin{align}
  \tilde{\mu}_i & = \sum_{m=0}^M \tilde{\beta}_{j_i^m}^{(m)} \\
  & = \sum_{m=0}^M r_i^{(m)} - r_i^{(m+1)} \\
  & = r_i^{(0)} - r_i^{(M+1)} \\
  & = \mu_i
\end{align}
...
# Priors for Main Effects and Interactions {#sec:priors}
...
\begin{align}
  \tau_m & \sim \text{N}^+\left(0, A_{\tau}^{(m)2}\right) \\
  \beta_1^{(m)} & \sim \text{N}(0, 1) \\
  \beta_j^{(m)} & \sim \text{N}(\beta_{j-1}^{(m)}, 1), \quad j = 2, \cdots, J_m
\end{align}
...
## Independent Autoregressive {#sec:pr-iar}
...
## Independent Linear {#sec:pr-ilin}
...
\begin{align}
  \beta_j^{(m)} & \sim \text{N}(0, 1), \quad j = 1, \cdots, S_m - 1 \\
  \beta_{S_m}^{(m)} & = - \sum_{j=1}^{S_m - 1} \beta_j^{(m)} \\
...
\begin{equation}
  \text{N}(\tau_m | 0, A_{\tau}^{(m)2}) \left( \prod_{j=1}^{S_m-1} \text{N}(\beta_j^{(m)} \mid 0, 1 ) \right)  \left( \prod_{j=S_m+1}^{J_m} \text{N}(\beta_j^{(m)} \mid \beta_{j-S_m}^{(m)}, \tau_m^2) \right) 
\end{equation}
...
# Standardization - second version

NOTE - ignoring covariates for the moment. I suspect we can just subtract the covariates term from $\pmb{\mu}$ and then proceed as before.

Under our model for means
\begin{equation}
 \pmb{\mu} = \sum_{m=0}^{(m)} \pmb{X}^{(m)} \pmb{\beta^{(m)}}
\end{equation}
where each $\pmb{X}^{(m)}$ is an incidence matrix of 0s and 1s.

Let
\begin{equation}
  \pmb{A}_m = (\pmb{X}^{(m)\top} \pmb{X}^{(m)})^{-1} \pmb{X}^{(m)\top}
\end{equation}
and let
\begin{align}
  \pmb{P}_0 & = \pmb{A}_0 \\
  \pmb{P}_m & = \pmb{A}_m (\pmb{I} - \pmb{X}^{(m-1)} \pmb{P}_{m-1}), \quad m = 1, \cdots, M.
\end{align}
We set
\begin{equation}
  \tilde{\pmb{\beta}}^{(m)} = \pmb{P}_m \pmb{\mu}
\end{equation}

Note that
\begin{align}
  \sum_{m=0}^M \pmb{X}^{(m)} \tilde{\pmb{\beta}}^{(m)} & =   \sum_{m=0}^M \pmb{X}^{(m)} \pmb{P}_m \pmb{\mu} \\
  & =
\end{align}
