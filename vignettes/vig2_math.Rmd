...
models <- tibble(
  ...
  `Data model` = c(
    ...
    paste(
      "$$",
      "w_i^{\\text{obs}} \\sim \\text{IG}\\left(\\frac{1}{\\varphi}, ",
      "\\left[\\frac{1}{\\varphi} - 1\\right] \\rho_i w_i^{\\text{true}} \\right)",
      "$$"
    ),
...
Unaffected: \@ref(eq:lik-pois-1)--\@ref(eq:lik-pois-2).
...
As shown in Appendix \@ref(app:datamod), \@ref(eq:lik-pois-1) and \@ref(eq:datamod-u-i) imply that
\begin{equation}
...
\begin{align}
    p(y | u) & \propto p(u | y) p(y) \\
    & = \text{Binomial}(u | y, \pi) \text{Poisson}(y | \lambda) \\
    & \propto \binom{y}{u}(1-\pi)^{y-u} \frac{1}{y!} \lambda^y \\
    & \propto \frac{1}{(y-u)!} [(1-\pi)\lambda]^y (\#eq:unnormalised)
\end{align}
The quantity $p(y|u)$ can be obtained by normalizing (\#eq:unnormalised). Summing \@ref(eq:unnormalised) over all possible values, we obtain
\begin{align}
    \sum_{y=u}^{\infty} \frac{1}{(y-u)!} [(1-\pi)\lambda]^y & = \sum_{i = 0}^{\infty} \frac{1}{i!}[(1-\pi)\lambda]^{i + u} \\
    & = [(1-\pi)\lambda]^u \sum_{i = 0}^{\infty} \frac{[(1-\pi)\lambda]^i}{i!} \\
    & = [(1-\pi)\lambda]^u e^{(1-\pi)\lambda} (\#eq:sum)
\end{align}
 by dividing \@ref(eq:unnormalised) by \@ref(eq:sum) gives
\begin{equation}
  p(y | u) = \frac{[(1-\pi)\lambda]^{y-u} e^{-(1-\pi)\lambda}}{y-u}. (\#eq:pmf)
\end{equation}
