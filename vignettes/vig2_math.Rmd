...
\begin{equation}
  \text{N}(\tau_m | 0, A_{\tau}^{(m)2}) \text{N}(\eta^{(m)} \mid 0, 0.001)  \prod_{j=2}^{J_m} \text{N}\left(\beta_j^{(m)} - \beta_{j-1}^{(m)} \mid 0, \tau_m^2 \right) 
\end{equation}
...
\begin{align}
 \eta_0^{(m)} & \sim \text{N}(0, 0.001) \\
 \eta_1^{(m)} & \sim \text{N}(0, 0.001)
\end{align}
where
\begin{align}
...
\begin{equation}
  \text{N}(\tau_m | 0, A_{\tau}^{(m)2}) \text{N}(\eta_0^{(m)} \mid 0, 0.001) \text{N}(\eta_1^{(m)} \mid 0, 0.001) \prod_{j=3}^{J_m} \text{N}\left(\beta_j^{(m)} - 2 \beta_{j-1}^{(m)} + \beta_{j-2}^{(m)} \mid 0, \tau_m^2 \right) 
\end{equation}
