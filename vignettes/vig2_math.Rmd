...
\begin{equation}
  \text{N}^+\left(\tau_m | 0, A_{\tau}^{(m)2} \right) \text{Beta}\left( r^{(m)} \mid 2, 2 \right) p\left( \beta_1^{(m)}, \cdots, \beta_{J_m}^{(m)} | \phi_1^{(m)}, \cdots, \phi_{K_m}^{(m)}, \tau_m \right) 
\end{equation}
...
\begin{equation}
  \text{N}^+\left(\tau_m | 0, A_{\tau}^{(m)2} \right) \text{Beta}\left( r^{(m)} \mid 2, 2 \right) \prod_{u=1}^{U_m} p\left( \beta_{u,1}^{(m)}, \cdots, \beta_{u,V_m}^{(m)} | \phi_1^{(m)}, \cdots, \phi_{K_m}^{(m)}, \tau_m \right) 
\end{equation}
...
\begin{align}
  \beta_{uv}^{(m)} & \sim \text{N}(q_v^{(m)} \eta_u^{(m)}, \tau_m^2) \\
  \eta_u^{(m)} & \sim \text{N}\left(0, A_{\eta}^{(m)2}\right) \\
  \tau_m & \sim \text{N}^+\left(0, A_{\tau}^{(m)2}\right)
\end{align}
where
\begin{equation}
...
\begin{equation}
  \text{N}(\tau_m | 0, A_{\tau}^{(m)2}) 
  \quad \times \prod_{u=1}^{U_m} \text{N}\left(\eta_u^{(m)} \mid 0, A_{\eta}^{(m)2} \right) 
...
\begin{align}
  \beta_j^{(m)} & = q_j^{(m)} \eta^{(m)} + \epsilon_j^{(m)} \\
  \epsilon_j^{(m)} & \sim \text{N}\left(\phi_1^{(m)} \epsilon_{j-1}^{(m)} + \cdots + \phi_{K_m}^{(m)} \epsilon_{j-{K_m}}^{(m)},  \omega_m^2\right), \quad j = K_m + 1, \cdots, J_m. \\
  \eta^{(m)} & \sim \text{N}\left(0, A_{\eta}^{(m)2}\right)
\end{align}
where
\begin{equation}
...
Internally, TMB derives values for $\omega_m$ and $\epsilon_1^{(m)}, \cdots, \epsilon_{K_m -1}^{(m)}$ that give every error term $\epsilon_j^{(m)}$ the same marginal variance. We denote this marginal variance $\tau_m^2$, and assign it a prior
\begin{equation}
  \tau_m  \sim \text{N}^+(0, A_{\tau}^{(m)2}).
\end{equation}
Each of the individual $\phi_k^{(m)}$ is restricted to the interval $(-1, 1)$, and the $\phi_k^{(m)}$ are jointly restricted to values that yield stationary models. Let
...
\begin{equation}
  \text{N}^+\left(\tau_m | 0, A_{\tau}^{(m)2} \right) \text{Beta}\left( r_k^{(m)} \mid 2, 2 \right) \text{N}(\eta^{(m)} | 0, A_{\eta}^{(m)2}) p\left( \epsilon_1^{(m)}, \cdots, \epsilon_{J_m}^{(m)} | \phi_1^{(m)}, \cdots, \phi_{K_m}^{(m)}, \tau_m \right) 
\end{equation}
where $p\left( \epsilon_1^{(m)}, \cdots, \epsilon_{J_m}^{(m)} | \phi_1^{(m)}, \cdots, \phi_{K_m}^{(m)}, \tau_m \right)$ is calculated internally by TMB.

### Forecasting

\begin{align}
  \beta_{J_m + h}^{(m)} & = q_{J_m + h} \eta^{(m)} + \epsilon_{J_m+h}^{(m)} \\
  \epsilon_{J_m+h}^{(m)} & \sim \text{N}\left(\phi_1^{(m)} \epsilon_{J_m + h - 1}^{(m)} + \cdots + \phi_{K_m}^{(m)} \epsilon_{J_m+h-K_m}^{(m)}, \tau_m^2\right)
\end{align}
...
Main effects with a P-Spline prior cannot be forecasted.
