...
models <- tibble(
  Model = c("Poisson", "Poisson", "Binomial", "Normal"),
  `System model` = c(
    paste(
      "$$",
    ...
    paste(
      ...
      "\\gamma_i \\sim \\text{Beta}(\\xi^{-1} \\mu_i, \\xi^{-1}(1 - \\mu_i))",
      "$$"
    ),
    "$$ y_i^{\\text{true}} \\sim \\text{N}(\\mu_i, w_i^{-1} \\xi^2) $$"
  ),
  `Data model` = c(
    paste(
      "$$",
    ...
    paste(
      "$$",
      "w_i^{\\text{obs}} \\sim \\text{InvGamma}(2 + \\delta_{g[i]}^{-1}, (1 + \\delta_{g[i]}^{-1}) r_{h[i]} w_i^{\\text{true}})",
      "$$"
    ),
    ...
    paste(
      ...
      "\\epsilon_i \\sim \\text{N}(m_{g[i]}, s_{h[i]}^2)",
      "$$"
    )
  )
)
...
## Poisson, measurement errors in outcome variable {#sec:datamod-pois-out}

### System model

Same as original model, but be explicit that modelling actual outcome:
\begin{align}
  y_i^{\text{true}} & \sim \text{Poisson}(\gamma_i w_i) (\#eq:lik-pois-outcome-1) \\
  \gamma_i & \sim \text{Gamma}\left(\xi^{-1}, (\mu_i \xi)^{-1}\right),  (\#eq:lik-pois-outcome-2)
\end{align}
with option that $\xi \equiv 0$ so that
\begin{equation}
  y_i^{\text{true}} \sim \text{Poisson}(\mu_i w_i). (\#eq:like-pois-outcome-3)
\end{equation}
...
Note
\begin{align}
  u_i & \mid \pi_{g[i]}, \gamma_i, w_i \sim \text{Poisson}(\pi_{g[i]} \gamma_i w_i), (\#eq:datamod-u-i-cond) \\
  y_i^{\text{obs}} & \mid \pi_{g[i]}, \kappa_{h[i]}, \gamma_i, w_i \sim \text{Poisson}((\pi_{g[i]} + \kappa_{h[i]}) \gamma_i w_i)
\end{align}


We allow for the possibility that $p_i \equiv 1$, $m_i > 0$, which yields a pure overcount model, and for $m_i \equiv 0$, $0 < p_i < 1$, which yields a pure undercount model.
...
\begin{equation}
  u_i \mid y_i^{\text{obs}}, \pi_{g[i]}, \kappa_{h[i]}, \mu_i, \xi, w_i \sim  \text{Binomial}\left(y_i^{\text{obs}}, \frac{\pi_{g[i]}}{\pi_{g[i]} + \kappa_{h[i]}}\right)
\end{equation}
...
\begin{equation}
  y_i^{\text{true}} \mid y_i^{\text{obs}}, \kappa_{h[i]}, \mu_i, \xi, w_i \sim \text{Binomial}\left(y_i^{\text{obs}}, \frac{1}{1 + \kappa_{h[i]}}\right)
\end{equation}
...
\begin{align}
  w_i^{\text{obs}} & \sim \text{InvGamma}(2 + \delta_{g[i]}^{-1}, (1 + \delta_{g[i]}^{-1}) r_{h[i]} w_i^{\text{true}}) \\
  \delta_g & \sim \text{Exp}(A_g^{(\delta)}) 
\end{align}
where values for $r_h$ and $A_g^{(\delta)}$ are supplied by the user.

Note that
\begin{align}
  E[w_i^{\text{obs}} \mid w_i^{\text{true}}] & = r_{h[i]} w_i^{\text{true}} \\
  \text{var}[w_i^{\text{obs}} \mid w_i^{\text{true}}] & = \delta_{g[i]} (r_{h[i]} w_i^{\text{true}})^2 \\
  w_i^{\text{true}} \mid w_i^{\text{obs}}  & \sim \text{Gamma}(3 + \delta_{g[i]}^{-1}, [1 + \delta_{g[i]}^{-1}] r_{h[i]} [w_i^{\text{obs}}]^{-1}) (\#eq:w-true) \\
  E[w_i^{\text{true}} \mid w_i^{\text{obs}}] & = \frac{3\delta_{g[i]} + 1}{\delta_{g[i]} + 1} \frac{w_i^{\text{obs}}}{r_{h[i]}}
\end{align}
...
\begin{align}
  y_i^{\text{obs}} & \sim \text{NegBinom}(3 + \delta_{g[i]}^{-1}, [1 + (1 + \delta_{g[i]}^{-1})^{-1}r_{h[i]}^{-1} \mu_i w_i^{\text{obs}}]^{-1} )  \\
  \delta_g & \sim \text{Exp}(A_g^{(\delta)})
\end{align}

Note that
\begin{equation}
E[y_i^{\text{obs}}] = \left( \frac{3 + \delta_{g[i]}^{-1}}{1 + \delta_{g[i]}^{-1}} \right) \frac{w_i^{\text{obs}}}{r_{h[i]}} \mu_i 
\end{equation}
...
\begin{equation}
  w_i^{\text{true}} \sim \text{Gamma}(3 + \delta_{g[i]}^{-1} + y_i, [1 + \delta_{g[i]}^{-1}] r_{h[i]} [w_i^{\text{obs}}]^{-1} + \mu_i)
\end{equation}
...
## Normal, measurement errors in outcome variable
...
\begin{align}
  ...
  \epsilon_i & \sim \text{N}(m_{g[i]}, s_{h[i]}^2)
\end{align}
where values for $m_{g[i]}$ and $s_{h[i]}$ are supplied by the user.

### Combined model seen by TMB

\begin{equation}
  \frac{y_i^{\text{obs}} - \bar{y}}{s} \sim \text{N}\left(\mu_i + \frac{m_{g[i]}}{s}, \frac{\bar{w} \xi^2}{w_i} + \frac{s_{h[i]}^2}{s^2}\right)
\end{equation}
...
\begin{equation}
  y_i^{\text{true}} \sim \text{N}\left(\frac{\tau_1}{\tau_1 + \tau_2} \gamma_i + \frac{\tau_2}{\tau_1 + \tau_2} (y_i^{\text{obs}} - m_{g[i]}),\quad
...
- $p_{\text{un}}(\cdot) = \text{NegBinom}\left(\cdot \mid 3 + \delta_{g[i]}^{-1}, [1 + (1 + \delta_{g[i]}^{-1})^{-1}r_{h[i]}^{-1} \mu_i w_i^{\text{obs}}]^{-1} \right)$ in Poisson model with measurement error in exposure;
...
Assume have no prior information on $w_i^{\text{true}}$, so that $w_i^{\text{true}} \sim \text{Unif}(0, \infty)$. Let $A = 2 + \delta_{g[i]}^{-1}$ and $B = (1 + \delta_{g[i]}^{-1}) r_{h[i]}$.
...
\begin{align}
  ...
  & = \text{Gamma}(w_i^{\text{true}} \mid 3 + \delta_{g[i]}^{-1}, (1 + \delta_{g[i]}^{-1}) r_{h[i]} [w_i^{\text{obs}}]^{-1})
\end{align}
as required.
