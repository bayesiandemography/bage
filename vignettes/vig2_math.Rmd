...
models <- tibble(
  ...
  `Data model` = c(
    ...
    paste(
      "$$",
      "w_i^{\\text{obs}} \\sim \\text{InvGamma}(2 + \\delta^{-1}, (1 + \\delta^{-1}) r_i w_i^{\\text{true}})",
      "$$"
    ),
...
## Poisson, measurement errors in outcome variable
...
\begin{align}
  ...
  E[y_i^{\text{obs}} - y_i^{\text{true}}] & = (\kappa_i - [1 - \pi_i]) \gamma_i w_i \\
  \text{var}[y_i^{\text{obs}} - y_i^{\text{true}}] & = (\kappa_i + [1 -\pi_i]) \gamma_i w_i
\end{align}
...
\begin{align}
  ...
  \pi_g & \sim \text{Beta}(A_g^{(\pi)}, B_g^{(\pi)}) \\
  \kappa_h & \sim \text{Gamma}(A_h^{(\pi)}, B_h^{(\pi)})
\end{align}


### Deriving $y_i^{\text{true}}$

\@ref(eq:datamod-v-i) and \@ref(eq:datamod-u-i-cond) imply that
\begin{equation}
  u_i \mid y_i^{\text{obs}}, \pi_{g[i]}, \kappa_{h[i]}, \gamma_i, w_i \sim \text{Binomial}\left(y_i^{\text{obs}}, \frac{\pi_{g[i]}}{\pi_{g[i]} + \kappa_{h[i]}}\right)
\end{equation}

As shown in Appendix \@ref(app:datamod), \@ref(eq:lik-pois-outcome-1) and \@ref(eq:datamod-u-i) imply that
\begin{equation}
  y_i^{\text{true}} - u_i \mid u_i, \pi_{g[i]}, \gamma_i, w_i \sim \text{Poisson}((1 - \pi_{g[i]}) \gamma_i w_i)
\end{equation}
...
\begin{align}
  w_i^{\text{obs}} & \sim \text{InvGamma}(2 + \delta^{-1}, (1 + \delta^{-1}) r_i w_i^{\text{true}}) \\
  \delta & \sim \text{Exp}(A_{\delta}) 
\end{align}
where values for $r_i$ and $A_{\delta}$ are supplied by the user.

Note that
\begin{align}
  E[w_i^{\text{obs}} \mid w_i^{\text{true}}] & = r_i w_i^{\text{true}} \\
  \text{var}[w_i^{\text{obs}} \mid w_i^{\text{true}}] & = \delta (r_i w_i^{\text{true}})^2 \\
  w_i^{\text{true}} \mid w_i^{\text{obs}}  & \sim \text{Gamma}(3 + \delta^{-1}, (1 + \delta^{-1}) r_i [w_i^{\text{obs}}]^{-1}) (\#eq:w-true) \\
  E[w_i^{\text{true}} \mid w_i^{\text{obs}}] & = \frac{3\delta + 1}{\delta + 1} \frac{w_i^{\text{obs}}}{r_i} \\
  \text{var}[w_i^{\text{true}} \mid w_i^{\text{obs}}] & = \frac{\delta (3\delta + 1)}{(\delta + 1)^2} \left(\frac{w_i^{\text{obs}}}{r_i}\right)^2
\end{align}

(\@ref(eq:w-true)) derived in Appendix \@ref(app:datamod))

### Combined model seen by TMB

\begin{align}
  y_i^{\text{obs}} & \sim \text{NegBinom}(3 + \delta^{-1}, [1 + (1 + \delta^{-1})^{-1}r_i^{-1} \mu_i w_i^{\text{obs}}]^{-1} ) \\
  \delta & \sim \text{Exp}(A_{\delta})
\end{align}

Note that
\begin{align}
E[y_i^{\text{obs}}] &= \left( \frac{3 + \delta^{-1}}{1 + \delta^{-1}} \right) \cdot \frac{\mu_i w_i^{\text{obs}}}{r_i} \\
\\
\text{var}(y_i^{\text{obs}}) &= \left( \frac{3 + \delta^{-1}}{1 + \delta^{-1}} \right) \cdot \frac{\mu_i w_i^{\text{obs}}}{r_i} \cdot \left( 1 + \left(1 + \delta^{-1}\right)^{-1} \cdot \frac{\mu_i w_i^{\text{obs}}}{r_i} \right)
\end{align}
...
\begin{align}
  ...
  \epsilon_i & \sim \text{N}(m_i, s_i^2)
\end{align}
where values for $m_i$ and $s_i$ are supplied by the user.

### Combined model seen by TMB

\begin{equation}
  y_i^{\text{obs}} \sim \text{N}(\gamma_i + m_i, w_i^{-1}\sigma^2 + s_i^2)
\end{equation}
...
\begin{equation}
  y_i^{\text{true}} \sim \text{N}\left(\frac{\tau_1}{\tau_1 + \tau_2} \gamma_i + \frac{\tau_2}{\tau_1 + \tau_2} (y_i^{\text{obs}} - m_i),\quad
\left( \tau_1 + \tau_2 \right)^{-1} \right)
\end{equation}
where $\tau_1 = \frac{w_i}{\sigma^2}$ and $\tau_2 = \frac{1}{s_i^2}$
...
\begin{align}
  p(y_i^{\text{con}}) & = \sum_{y_i^{\text{un}}} p_{\text{con}}(y_i^{\text{con}} | y_i^{\text{un}}) p_{\text{un}}(y_i^{\text{un}}) \\
   & =  \sum_{k = -2, -1, 0, 1, 2} p_{\text{con}}(y_i^{\text{con}} | y_i^{\text{con}} + k) p_{\text{un}}(y_i^{\text{con}} + k)
\end{align}
where
...
- $p_{\text{un}}(\cdot) = \text{NegBinom}\left(\cdot \mid 3 + \delta^{-1}, [1 + (1 + \delta^{-1})^{-1}r_i^{-1} \mu_i w_i^{\text{obs}}]^{-1} \right)$ in Poisson model with measurement error in exposure;
- $p_{\text{un}}(\cdot) = \text{BetaBinomial}(\cdot \mid w_i, \xi^{-1} \mu_i, \xi^{-1} [1-\mu_i])$ in Binomial model with not measurement error; and
...
### Derivation of $y_i^{\text{true}} | u_i$

\begin{align}
    p(y_i^{\text{true}} \mid u_i) & \propto p(u_i \mid y_i^{\text{true}}) p(y_i^{\text{true}}) \\
    & = \text{Binomial}(u_i \mid y_i^{\text{true}}, \pi_i) \text{Poisson}(y_i^{\text{true}} \mid \lambda) \\
    & \propto \binom{y_i^{\text{true}}}{u_i}(1-\pi_i)^{y_i^{\text{true}}-u_i} \frac{1}{y_i^{\text{true}}!} \lambda^{y_i^{\text{true}}} \\
    & \propto \frac{1}{(y_i^{\text{true}}-u_i)!} [(1-\pi_i)\lambda]^{y_i^{\text{true}}} (\#eq:unnormalised)
\end{align}
The quantity $p(y_i^{\text{true}} \mid u_i)$ can be obtained by normalizing \@ref(eq:unnormalised). Summing \@ref(eq:unnormalised) over all possible values, we obtain
\begin{align}
    \sum_{y_i^{\text{true}}=u_i}^{\infty} \frac{1}{(y_i^{\text{true}}-u_i)!} [(1-\pi_i)\lambda]^{y_i^{\text{true}}} & = \sum_{i = 0}^{\infty} \frac{1}{i!}[(1-\pi_i)\lambda]^{i + u_i} \\
    & = [(1-\pi_i)\lambda]^u_i \sum_{i = 0}^{\infty} \frac{[(1-\pi_i)\lambda]^i}{i!} \\
    & = [(1-\pi_i)\lambda]^u_i e^{(1-\pi_i)\lambda} (\#eq:sum)
\end{align}
 by dividing \@ref(eq:unnormalised) by \@ref(eq:sum) gives
\begin{equation}
  p(y_i^{\text{true}} \mid u_i) = \frac{[(1-\pi_i)\lambda]^{y_i^{\text{true}}-u_i} e^{-(1-\pi_i)\lambda}}{y_i^{\text{true}}-u_i}. (\#eq:pmf)
\end{equation}


### Derivation of $p(w_i^{\text{true}} \mid w_i^{\text{obs}})$

Assume have no prior information on $w_i^{\text{true}}$, so that $w_i^{\text{true}} \sim \text{Unif}(0, \infty)$. Let $A = 2 + \delta^{-1}$ and $B = (1 + \delta^{-1}) r_i$.
...
\begin{align}
  ...
  & = \text{Gamma}(w_i^{\text{true}} \mid 3 + \delta^{-1}, (1 + \delta^{-1}) r_i [w_i^{\text{obs}}]^{-1})
\end{align}
as required.
