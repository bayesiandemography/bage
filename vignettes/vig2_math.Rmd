...
models <- tibble(
  ...
  `Data model` = c(
    ...
    paste(
      "$$",
      "w_i^{\\text{obs}} \\sim \\text{InvGamma}(2 + \\delta_{g[i]}^{-1}, [1 + \\delta_{g[i]}^{-1}] w_i^{\\text{true}})",
      "$$"
    ),
    paste(
    "$$",
...
## Poisson, overcount in outcome variable {#sec:datamod-pois-overcount}
...
## Poisson, undercount and overcount outcome variable {#sec:datamod-pois-miscount}
...
\begin{align}
  ...
  v_i & \sim \text{Poisson}(\kappa_{h[i]} \gamma_i w_i) (\#eq:datamod-v-i) \\
  \pi_g & \sim \text{Beta}(a_g^{(\pi)}, b_g^{(\pi)}) \\ 
  \kappa_h & \sim \text{Gamma}(a_h^{(\kappa)}, b_h^{(\kappa)}),
\end{align}
...
As shown in Appendix \@ref(app:datamod), equations \@ref(eq:datamod-v-i) and \@ref(eq:datamod-u-i-cond) imply that
\begin{equation}
  u_i \mid y_i^{\text{obs}}, \pi_{g[i]}, \kappa_i, \mu_i, \xi, w_i \sim  \text{Binomial}\left(y_i^{\text{obs}}, \frac{\pi_{g[i]}}{\pi_{g[i]} + \kappa_i}\right)
\end{equation}

As also shown in Appendix \@ref(app:datamod), equations \@ref(eq:lik-pois-outcome-1), \@ref(eq:lik-pois-outcome-2), and \@ref(eq:datamod-u-i) imply that
\begin{equation}
...
- $p_{\text{un}}(\cdot) = \text{NegBinom}\left(\cdot \mid \xi^{-1}, [1 + (\pi_{g[i]} + \kappa_{h[i]}) \mu_i w_i \xi]^{-1}\right)$ in Poisson model with measurement error in outcome;
- $p_{\text{un}}(\cdot) = \text{NegBinom}\left(\cdot \mid 3 + d_{g[i]}^{-1}, [1 + (1 + d_{g[i]}^{-1})^{-1}^{-1} \mu_i w_i^{\text{obs}}]^{-1} \right)$ in Poisson model with measurement error in exposure;
...
### Derivation of $u_i \mid y_i^{\text{obs}}, \pi_{g[i]}, \kappa_{h[i]}, \mu_i, \xi, w_i$ in undercount-overcount model in Section \@ref(sec:datamod-pois-out)
...
### Derivation of $y_i^{\text{true}} \mid u_i, \pi_{g[i]}, \kappa_{h[i]}, \mu_i, w_i, \xi$ in undercount-overcount model Section \@ref(sec:datamod-pois-out)
...
### Derivation of $y_i^{\text{true}} \mid y_i^{\text{obs}}, \pi_{g[i]}, \mu_i, \xi, w_i$ in undercount-only model in Section \@ref(sec:datamod-pois-out)
...
### Derivation of $y_i^{\text{true}} \mid y_i^{\text{obs}}, \kappa_{h[i]}, \mu_i, \xi, w_i$ in overcount-only model in Section \@ref(sec:datamod-pois-out)
...
### Derivation of $y_i^{\text{true}} \mid y_i^{\text{obs}}, \pi_{g[i]}, \mu_i, \xi, w_i$ in binomial undercount model in Section \@ref(sec:datamod-binom)
