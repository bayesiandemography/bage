...
\begin{align}
  \beta_{u,1}^{(m)} & = 0, \quad u = 1, \cdots, U_m \\
...
If sum-to-zero constraints are being used, extract means for each of the 'by' variables, within each category of the 'along' variable.
...
\begin{align}
  \beta_{u,1}^{(m)} & = 0, \quad u = 1,\cdots,U_m  \\
  \beta_{u,2}^{(m)} & \sim \text{N}\left(0, (A_{\eta}^{(m)})^2\right), \quad u = 1,\cdots, U_m \\
  \beta_{u,v}^{(m)} & \sim \text{N}\left(2 \beta_{u,v-1}^{(m)} - \beta_{u,v-2}^{(m)}, \tau_m^2\right), \quad u = 1, \cdots, U_m, \quad v = 3, \cdots, V_m \\
  \tau_m & \sim \text{N}^+\left(0, (A_{\tau}^{(m)})^2\right)
\end{align}
...
\begin{align}
  \tau_m & \sim \text{N}^+\left(0, A_{\tau}^{(m)2}\right) \\
  \beta_{u,2}^{(m)} & \sim \text{N}(0, (A_{\eta}^{(m)})^2), \quad u = 1, \cdots, U_m \\
  \beta_{u,3}^{(m)} & \sim \text{N}(2 \beta_{u,2}^{(m)}, \tau_m^2), \quad u = 1, \cdots, U_m, \quad v = 3,\cdots,V_m
  \beta_{u,v}^{(m)} & \sim \text{N}(2 \beta_{u,v-1}^{(m)} - \beta_{u,v-2}^{(m)}, \tau_m^2), \quad u = 1, \cdots, U_m, \quad v = 4,\cdots,V_m
\end{align}
...
## Second-order random walk, with seasonal effect {#sec:rw2seas}
