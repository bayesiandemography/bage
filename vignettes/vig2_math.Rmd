...
Priors for the $\pmb{\beta}^{(m)}$ are described in sections \@ref(sec:pr-int)--\@ref(sec:pr-interact-high). Covariate matrix $\pmb{Z}$ and the prior for vector $\pmb{\zeta}$ are discussed in Section \@ref(‭sec:cov).
...
\end{align}
where $\pmb{\Phi}^{(m)}$ is a $J_m \times K_m$ matrix. In AR1 priors (Section \@ref(sec:pr-ar1)) and linear priors with AR1 errors (Section \@ref(sec:pr-lin-ar1)), $\pmb{\Phi}^{(m)}$ depends on correlation coefficient $\phi_m$. In all other priors, $\pmb{\Phi}^{(m)}$ is fixed and known.
...
\begin{align}
  \beta^{(m)} & \sim \text{N}\left(\pmb{0}, c_m \tau_m^2 \pmb{V}^{(m)} \right) \\
  \tau_m & \sim \text{N}^+(A_m^2)
\end{align}
...
\begin{align}
  \pmb{\Phi}^{(m)} & = \begin{bmatrix} \pmb{l}^{(m)} & \pmb{D}_{2m}^{\top} (\pmb{D}_{2m} \pmb{D}_{2m}^{\top})^{-1} \end{bmatrix} \\
  K_m & = J_m - 1 \\
  \nu_1^{(m)} & = L_m^2 \\
...
- `sd` is $L_m$. Defaults to $s$, as defined in Equation \@ref(eq:defn-s), if user does not supply a value.
...
\begin{align}
  \pmb{\Phi}^{(m)} & = \begin{bmatrix} \pmb{l}^{(m)} & \pmb{I}_{J_m} \end{bmatrix} \\
  K_m & = J_m + 1\\
  \nu_1^{(m)} & =  L_m^2 \\
...
\begin{align}
  \pmb{\Phi}^{(m)} & = \begin{bmatrix} \pmb{l}^{(m)} & \pmb{L}^{(m)} \end{bmatrix} \\
  K_m & = J_m + 1 \\
  \nu_1^{(m)} & =  L_m^2 \\
  \nu_k^{(m)} & =  \tau_m, \quad k = 2, \cdots, K_m
\end{align}
where $\pmb{l}^{(m)}$ is defined as in Section \@ref(sec:pr-rw2), and $\pmb{L}^{(m)}$ is defined as in Section \@ref(sec:pr-ar1).
...
\begin{align}
  \pmb{\Phi}^{(m)} & = \begin{bmatrix} \pmb{B}^{(m)} \pmb{l}^{(m)} & \pmb{B}^{(m)} \pmb{D}_{2m}^{\top} (\pmb{D}_{2m} \pmb{D}_{2m}^{\top})^{-1} \end{bmatrix} \\
  K_m & = G_m - 1 \\
  \nu_1 & = L_m^2 \\
  \nu_k &  = c_m \tau_m, \quad k = 2, \cdots, K_m,
\end{align}
where $\pmb{l}^{(m)}$ is a vector with $G_m$ elements, the $g$th element of which is
\begin{equation}
...
\begin{equation}
  \pmb{B}^{(m)} \pmb{\zeta}^{(m)} = \pmb{B}^{(m)} \begin{bmatrix} \pmb{l}^{(m)} & \pmb{D}_{2m}^{\top} (\pmb{D}_{2m} \pmb{D}_{2m}^{\top})^{-1} \end{bmatrix}
  \begin{bmatrix} \lambda_m \\ \pmb{\epsilon}^{(m)} \end{bmatrix}.
\end{equation}
...
| $c_k$ | Scaling factor applied to variance terms in priors. | 
| $\lambda^{(m)}$ | Parameter specific to $m$th main effect or interaction. |
| $L_m$ | Standard deviation in normal prior for $\lambda_m$. |
