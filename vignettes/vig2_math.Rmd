...
\begin{equation}
  \text{N}(\tau_m \mid 0, A_{\tau}^{(m)2}) \prod_{u=1}^{U_m} \text{N}(\beta_{u,2}^{(m)} \mid 0, \tau_m^2)  \prod_{v=3}^{V_m} \text{N}\left(\beta_{uv}^{(m)} \mid \beta_{u,v-1}^{(m)}, \tau_m^2 \right) 
\end{equation}
...
\begin{equation}
  \text{N}(\tau_m \mid 0, A_{\tau}^{(m)2}) \prod_{u=1}^{U_m} \text{N}(\beta_{u,2}^{(m)} \mid 0, (A_{\eta}^{(m)})^2) \text{N}(\beta_{u,3}^{(m)} \mid 2 \beta_{u,2}^{(m)}, \tau_m^2) \prod_{v=4}^{V_m} \text{N}\left(\beta_{u,v}^{(m)} \mid 2 \beta_{u,v-1}^{(m)} - \beta_{u,v-2}^{(m)}, \tau_m^2 \right) 
\end{equation}
...
```
RW2(s = 1, sd = 1, along = NULL, zero_sum = TRUE)
```
...
\begin{align}
  \beta_{u,v}^{(m)} & = \alpha_{u,v} + \lambda_{u,s_v} \\
  \alpha_{u,1}^{(m)} & = 0 \\
...
Scale parameter $A_{\omega}^{(m)2}$ can be set to zero, implying that seasonal effect for each season does not change over time.
...
\begin{split}
  ...
  \quad \times & \prod_{u=1}^{U_m} \left( \text{N}(\alpha_{u,2}^{(m)} \mid 0, \tau_m^2 ) \prod_{v=3}^{V_m} \text{N}(\alpha_{u,v}^{(m)} \mid \alpha_{u,v-1}^{(m)}, \tau_m^2 ) \prod_{v=1}^{S_m-1}  \text{N}(\lambda_{u,v}^{(m)} \mid 0, (A_{\lambda}^{(m)})^2) \\
  \prod_{v > S_m, \; v \;\text{mod}\; S_m \neq 0}^{V_m} \text{N}(\lambda_{u,v}^{(m)} \mid \lambda_{u,v-S_m}^{(m)}, \omega_m^2) \right)
\end{split}
\end{equation}
...
```
RW_Seas(n, s = 1, s_seas = 1, sd_seas, along = NULL, zero_sum = FALSE)
```
...
\begin{align}
  \beta_{u,v}^{(m)} & = \alpha_{u,v} + \lambda_{u,s_v} \\
  \alpha_{u,1}^{(m)} & = 0 \\
  \alpha_{u,2}^{(m)} & \sim \text{N}\left(0, (A_{\eta}^{(m)})^2\right) \\
...
\begin{split}
  & \text{N}(\tau_m \mid 0, A_{\tau}^{(m)2}) \text{N}(\omega_m \mid 0, A_{\omega}^{(m)2}) \\
  \quad \times & \prod_{u=1}^{U_m} \left( \text{N}(\alpha_{u,2}^{(m)} \mid 0, (A_{\eta}^{(m)})^2 ) \prod_{v=3}^{V_m} \text{N}(\alpha_{u,v}^{(m)} \mid 2 \alpha_{u,v-1}^{(m)} - \alpha_{u,v-2}^{(m)}, \tau_m^2 ) \prod_{v=1}^{S_m-1}  \text{N}(\lambda_{u,v}^{(m)} \mid 0, (A_{\lambda}^{(m)})^2) \\
  \prod_{v > S_m, \; v \;\text{mod}\; S_m \neq 0}^{V_m} \text{N}(\lambda_{u,v}^{(m)} \mid \lambda_{u,v-S_m}^{(m)}, \omega_m^2) \right)
  \end{split}
\end{equation}
...
```
RW2_Seas(n, s = 1, s_seas = 1, along = NULL)
```
...
\end{equation}
Each of the individual $\phi_k^{(m)}$ is restricted to the interval $(-1, 1)$, and the $\phi_k^{(m)}$ are jointly restricted to values that yield stationary models. We implement the restriction indirectly. Let
\begin{equation}
  \pi_1^{(m)}, \cdots, \pi_{K_m}^{(m)}
\end{equation}
be the partial autocorrelation function (PACF) derived from $\phi_1^{(m)}, \cdots, \phi_{K_m}^{(m)}$ through the Durbin-Levinson recursion. We assign each $\pi_k^{(m)}$ the prior
\begin{equation}
  \frac{\pi_k^{(m)} + 1}{2} \sim \text{Beta}(2, 2).
\end{equation}
...
\begin{equation}
  \text{N}^+\left(\tau_m \mid 0, A_{\tau}^{(m)2} \right) \prod_{k=1}^{K_m} \text{Beta}\left(\tfrac{1}{2} \pi_k^{(m)} + \tfrac{1}{2} \mid 2, 2 \right) \prod_{u=1}^{U_m} p\left( \beta_{u,1}^{(m)}, \cdots, \beta_{u,V_m}^{(m)} \mid \phi_1^{(m)}, \cdots, \phi_{K_m}^{(m)}, \tau_m \right) 
\end{equation}
...
\begin{align}
  \beta_{u,v}^{(m)} & \sim \text{N}(\alpha_u^{(m)} + v \eta_u^{(m)}, \tau_m^2), \quad u = 1,\cdots,U_m, \quad v = 1, \cdots, V_m  \\
  \eta_u^{(m)} & \sim \text{N}\left(B_{\eta}^{(m)}, (A_{\eta}^{(m)})^2\right), \quad u = 1, \cdots, U_m \\
  \alpha_u^{(m)} & = -\frac{V_m + 1}{2} \eta_u^{(m)}, \quad u = 1,\cdots,U_m  \\
  \tau_m & \sim \text{N}^+\left(0, (A_{\tau}^{(m)})^2\right)
\end{align}
where $B_{\eta}^{(m)}$ and $A_{\eta}^{(m)}$ default to 0 and 1, though alternatives can be supplied by the user, or via informative priors (for instance, $B_{\eta}^{(m)} = -0.01$, or similar, for mortality)

Note that with the new version
\begin{equation}
 E \sum_{v=1}^{V_m} \beta_{u,v}^{(m)} = \sum_{v=1}^{V_m}\left(-\frac{V_m + 1}{2} \eta_u^{(m)} + v \eta_u^{(m)}\right) = 0
\end{equation}
...
\begin{equation}
  \text{N}^+\left(\tau_m \mid 0, A_{\tau}^{(m)2} \right)  \prod_{k=1}^{K_m} \text{Beta}\left( \tfrac{1}{2} \pi_k^{(m)} + \tfrac{1}{2} \mid 2, 2 \right) \prod_{u=1}^{U_m} \text{N}(\alpha_u^{(m)} \mid 0, 1) \text{N}(\eta_u^{(m)} \mid 0, A_{\eta}^{(m)2}) p\left( \epsilon_{u,1}^{(m)}, \cdots, \epsilon_{u,V_m}^{(m)} \mid \phi_1^{(m)}, \cdots, \phi_{K_m}^{(m)}, \tau_m \right) 
\end{equation}
