...
We forecast mortality rates through to 2027.  By default, the `forecast()` function returns output that looks like a return value from `augment()`. When `include_estimates` is `TRUE, the output includes historical values.

```{r}
vals_forecast <- mod |>
  forecast(labels = 2018:2027,
