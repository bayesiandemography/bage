...
\begin{equation}
  g(\pmb{\mu}) = \sum_{m=0}^{M} \pmb{X}^{(m)} \pmb{\beta}^{(m)} + \pmb{Z} \pmb{\zeta} + \pmb{o}
\end{equation}
where

- $g$ is log in Poisson models, logit in binomial models, and identity in normal and log-normal models;
- $\pmb{\beta}^{(m)}$, $m=1,\cdots,M$  denote a main effect or interaction with $J_m$ elements;
- $\pmb{\beta}^{(0)}$ is an intercept;
- $\pmb{X}^{(m)}$ be an $n \times J_m$ matrix of 1s and 0s, in which row $i$ picks out the element of $\beta^{m}$ that are used with cell $i$;
- \pmb{Z} is a $n \times P$ matrix of covariates;
- \pmb{\zeta} is a vector with $P$ elements; and
- \pmb{o} is an offset of known values.

Priors for the $\pmb{\beta}${(m)}$ are described in sections \@ref(sec:pr-int)--\@ref(sec:pr-interact), and the prior for $\pmb{\zeta} is discussed in Section \@ref(‭sec:pr_cov).
...
\end{equation}
where the linear trend is optional. We treat the independent normal prior (described in Section \@ref(sec:pr-inorm)) as a baseline. We require that, after removing any linear trend, the average value for the marginal variance of the $\beta_j^{(m)}$ is the same as it is for the independent normal prior:
\begin{equation}
...
\end{equation}
The default for $A_m$ is $s$, as defined in Equation \@ref(eq:defn-s). However, users can supply an alternative value.
...
# Specific priors for main effects {#sec:pr-main-specific}
...
\begin{align}
  \pmb{\beta}^{(m)} & = \pmb{D}_{2m}^{\top} (\pmb{D}_{2m} \pmb{D}_{2m}^{\top})^{-1} \pmb{\epsilon}^{(m)} + \lambda_m \pmb{l}^{(m)} \\
  \pmb{\epsilon}^{(m)} & \sim \text{N}(\pmb{0}, k_m \tau_m^2 \pmb{I}_{J_m - 2}) (\#eq:pr-rw2-eps) \\
  \lambda_m & \sim \text{N}(0, A_m^2) \\
  \tau_m & \sim \text{N}^+(A_m^2)
\end{align}
where $\pmb{D}_{2m}$ is a second-order difference matrix formed by multiplying two first-order difference matrices, and $\pmb{l}^{(m)}$ is a vector, the $j$th element of which is
\begin{equation}
  l_j^{(m)}  = -\frac{J_m + 1}{J_m - 1} + \frac{2}{J_m - 1} j.
\end{equation}
The $\lambda_m \pmb{l}^{(m)}$ term is needed because the columns of $\pmb{D}_{2m}^{\top} (\pmb{D}_{2m} \pmb{D}_{2m}^{\top})^{-1}$ do not span the space defined by $\sum_{j=1}^{J_m}{\beta_j^{(m)}} = 0$. See, for instance, @currie2002flexible [336].

We have $\pmb{D}_{2m} \pmb{\beta}^{(m)} =  \pmb{D}_{2m} \pmb{D}_{2m}^{\top} (\pmb{D}_{2m} \pmb{D}_{2m}^{\top})^{-1} \pmb{\epsilon}^{(m)} + \lambda_m \pmb{D}_{2m}^{\top} \pmb{l}^{(m)} = \pmb{\epsilon}^{(m)}$ and $\pmb{1}^{\top} \pmb{\beta}^{(m)} = 0$.
...
\begin{align}
  \pmb{\beta}^{(m)} & = \lambda_m \pmb{l}^{(m)} + \pmb{\epsilon}^{(m)} \\
...
\begin{equation}
  \pmb{\beta}^{(m)} = \pmb{X}^{(m)} \pmb{\alpha}^{(m)}
\end{equation}
where $\pmb{X}^{(m)} = (\pmb{x}_1^{(m)}(\pmb{j}), \cdots, \pmb{x}_{K_m}^{(m)}(\pmb{j}))$ is a $J_m \times K_m$ matrix of B-splines, with $\pmb{j} = (1, \cdots, J_m)^{\top}$. The B-splines are centered, so that $\pmb{1}^{\top} \pmb{x}_k^{(m)}(\pmb{j}) = 0$, $k = 1, \cdots K_m$.
...
\begin{align}
  ...
  \pmb{\alpha}^{(m)} & \sim \text{N}(\pmb{0}, \pmb{I}_{K_m})
\end{align}
where $\pmb{X}^{(m)}$ is a $J_m \times K_m$ matrix, and $\pmb{b}^{(m)}$ is a vector with $J_m$ elements, derived by performing an SVD on an external dataset of age-specific values, such as the Human Mortality Database, and then standardizing. The derivation of $\pmb{X}^{(m)}$ and $\pmb{b}^{(m)}$ is described in Appendix \@ref(app:svd-age).


### Code

`SVD(transform, translate = NULL)`

- `transform` is $\pmb{X}^{(m)}$
- `translate` is $\pmb{b}^{(m)}$. If no value is provided for `translate`, then it defaults to a vectors of zeros.
...
# Priors for coefficients for covariates {#sec:pr_cov}
